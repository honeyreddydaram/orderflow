...
public class KafkaConsumerConfig {
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, EventEnvelope<OrderConfirmedPayload>> orderConfirmedContainerFactory(
            KafkaTemplate<Object, Object> deadLetterKafkaTemplate) {
        var factory = new ConcurrentKafkaListenerContainerFactory<String, EventEnvelope<OrderConfirmedPayload>>();
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, EventEnvelope<OrderFailedPayload>> orderFailedContainerFactory(
            KafkaTemplate<Object, Object> deadLetterKafkaTemplate) {
        var factory = new ConcurrentKafkaListenerContainerFactory<String, EventEnvelope<OrderFailedPayload>>();
        factory.setConsumerFactory(consumerFactory(new TypeReference<>() {
        }));
        factory.setCommonErrorHandler(errorHandler(deadLetterKafkaTemplate));
        return factory;
